...
많은 사람들이 이 봇을 포크해서 사용할 경우, 모두 같은 시간(09:30)에 동행복권 서버에 접속하면 서버에 과부하를 줄 수 있습니다.

**해결책: 랜덤 딜레이**

봇은 시작 시 **0~5분(0~300초) 사이의 랜덤 딜레이**를 자동으로 추가합니다:

```python
# controller.py에서 자동 적용
...
**효과:**
- ✅ 1000명이 사용해도 트래픽이 5분에 걸쳐 분산 (초당 3~4명)
- ✅ 동행복권 서버 보호
- ✅ 모든 사용자에게 공정한 기회
- ✅ 윤리적이고 책임감 있는 사용
...
**실제 실행 시간:**
- Cron 시작: 09:30
- 실제 구매: 09:30 ~ 09:35 사이 랜덤

**로그 예시:**
```
[서버 부하 분산] 143초 후 구매 시작... (DDoS 방지)
[구매 시작] 사용자: abc***, 전략 모드: SUPERSTITION
```
